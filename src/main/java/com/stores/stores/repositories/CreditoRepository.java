...
public interface CreditoRepository extends CrudRepository<CreditoModel, Long> {
	@Query(value = "SELECT cr.* FROM clientes c, ventas v, creditos cr WHERE  c.nombre LIKE %:cedula% AND v.cliente_id=c.id AND v.tipo=1 AND cr.venta_id=v.id", nativeQuery = true)
